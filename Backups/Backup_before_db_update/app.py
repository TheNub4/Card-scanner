            ...
            with st.expander(label, expanded=(index == 0)):
                ...
                for i in range(0, len(pack_cards), columns_per_row):
                    ...
                    for col, pack_card in zip(cols, pack_cards[i:i + columns_per_row]):
                        code, qty, name, rarity, img = pack_card
...
st.subheader("Raw RiftCodex Card Data Check")

debug_set = st.text_input("Debug Set ID", value="UNL")
debug_card = st.text_input("Search card name/code", value="Diana")

if st.button("Fetch Raw Card Data"):
    try:
        url = f"https://api.riftcodex.com/cards?size=10&page=1&set_id={debug_set.lower()}&search={debug_card}"
        ...
        st.error(f"Could not fetch raw card data: {e}")
